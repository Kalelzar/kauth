...
pub fn initialize(allocator: std.mem.Allocator, comptime opts: m.RegistryOpts) !void {
    metrics = .{
        .total_memory_allocated = Metrics.MemUsage.init(
            "kauth_total_memory_bytes",
            .{ .help = "The total allocated memory by the server." },
            opts,
        ),
        .peak_memory_allocated = Metrics.MemUsage.init(
            "kauth_peak_memory_bytes",
            .{ .help = "The maximum allocated memory by the server." },
            opts,
        ),
        .statuses = try Metrics.Status.init(allocator, "kauth_https_statuses", .{}, opts),
    };
}
